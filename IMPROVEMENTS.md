...
## 2026-06-13 — main thread (/improve-quant) — P4 "what quants actually compute" round: real calculations across all 5 packages
- **Theme:** user asked for further improvements "in terms of calculation, etc that reflects what quants use/implement/need these days." Ran a fresh read-only `feature-architect` gap analysis per package (5 in parallel) benchmarked against the 2025–2026 OSS/desk standards (QuantLib/financepy, riskfolio/skfolio, quantstats/pyfolio + López de Prado, cryptofeed microstructure, ABIDES/mbt-gym/hftbacktest), confirmed the top-2 picks per package, then implemented them via specialist subagents. NEW branch `feature/quant-calculations` off `main` (per the post-2026-06-07 branch model). 6 conventional commits, **NOT pushed**. NO new dependencies anywhere; every existing public signature preserved; all default paths byte-identical.
- **Tooling note:** the first launch wave hit a background-agent write-permission denial, then an org spend-limit, then a model-availability error; re-ran the affected agents in the foreground on Opus 4.8. Net effect was only wasted attempts — no partial/corrupt edits landed (each blocked agent reported zero file changes). Main thread re-ran every suite + lint/format independently before/after committing.
- **options-pricing (vs QuantLib/financepy): 238 → 271 tests, 99.38% coverage.**
  - NEW `src/sabr.py` — Hagan (2002) lognormal SABR: `sabr_implied_vol` (vectorized over K, explicit ATM `z/x(z)→1` limit), `SABRParams`, `fit_sabr_slice(strikes, ivs, F, T, beta=0.5)` (bounded `least_squares` of (alpha,rho,nu), beta-fixed desk convention; mirrors `fit_svi_slice` so it fits `solve_iv_surface` slices), `sabr_smile`. Honest docs: Hagan approximation, NOT arbitrage-free, no negative-rate shift. SABR-vol→Black-76 demo.
  - NEW `src/finite_difference.py` — Crank-Nicolson θ=0.5 American/European pricer on a log-spot grid (Rannacher startup, `solve_banded` European step, PSOR early exercise w/ documented omega/tol/max_iter), `fd_price → FDResult(price, delta, gamma)` with grid Greeks. Self-validates vs `black_scholes` (European limit) and the CRR `BinomialTree` (American). Caught+fixed an O(1/n) node-bias by forcing ln(S) onto the center node (odd grid). Deferred: Heston/ADI/barriers/shifted-SABR.
- **portfolio-optimization (vs riskfolio/skfolio): 285 → 339 tests, 95.59% coverage.** (339 incl. the +12 shared-metrics tests from the backtesting commit below.)
  - NEW `risk_attribution(weights, groups=None) → DataFrame` (Euler MCR/CCR/%-risk, sector rollup via the existing `groups` convention; shares math with `portfolio_risk_contributions`; `PortfolioResult` untouched). Surfaced in `print_report`, FastAPI (`POST /risk-attribution`), Streamlit ("Risk attribution" tab).
  - NEW robust estimators (opt-in, defaults byte-identical, proven at atol 1e-12): `covariance.py` gains EWMA (RiskMetrics λ=0.94), OAS (Chen 2010 closed form), Marchenko-Pastur eigenvalue denoising (trace-preserving, requires T>n); `estimators.py` gains EWMA + James-Stein means. Threaded through `calculate_returns(*, cov_estimator, mean_estimator, ewma_lambda)`, config/CLI, FastAPI body fields, Streamlit dropdowns. `shrinkage`+`cov_estimator` together raises.
- **market-data (vs cryptofeed): 277 → 326 tests, 98.45% coverage.**
  - NEW opt-in trade-flow/VWAP bar enrichment (`ENABLE_BAR_FEATURES`/`--enable-bar-features`): per-bar buy/sell volume (exact taker side), signed imbalance, VWAP; `BarFeatures` computed in `TickNormalizer._build_bar`, persisted to a SEPARATE `bar_features` table (DuckDB+Timescale) + `barfeat:<symbol>` Redis publish. Default `ohlcv` rows byte-identical (proven).
  - NEW L2 book features on the existing `ENABLE_DEPTH` gate: `src/features.py` (microprice, quoted spread abs+bps, top-1/top-N depth imbalance, cumulative depth) computed in `_on_depth_message`, cached+published on `bookfeat:<symbol>`. Honest docs: snapshot-level only, NOT event-level OFI.
- **cpp/order-book (vs ABIDES/mbt-gym/hftbacktest): 53 C++ ctest unchanged (no C++ edits) + 60 → 89 Python tests, 95.39% coverage.** Both features pure Python over the existing pybind11 binding.
  - NEW L3 event tape (`TapeRecord`: ORDER/TRADE/CANCEL with sim time + taker/maker agent ids) + `KernelResult.agent_inventory`; NEW `python/orderbook/features.py` (microprice, spread, top-1/top-k volume imbalance, depth profile, snapshot — None-safe off the live engine).
  - NEW `AvellanedaStoikovAgent` (closed-form reservation skew + half-spread, user-supplied γ/k, online σ, POST_ONLY quoting, cancel-before-requote, inventory from REAL fills) on a kernel now supporting CANCEL events (latency-routed) + per-agent fill attribution via no-op-default hooks. `--compare-mm` demo (A-S vs naive maker). Default seed-42 demo byte-identical (subprocess parity test guards it). Honest docs: fills from the real engine, not a fill-intensity model.
- **Shared metrics (used by both backtesting + portfolio): PSR + DSR.**
  - Added `probabilistic_sharpe_ratio`, `deflated_sharpe_ratio`, `expected_max_sharpe` (Bailey–López de Prado closed forms, scipy-free via `math.erf`/`NormalDist`) to `portfolio_optimization_engine/metrics.py`. **Purely additive — git diff has zero `-def` lines; no existing signature changed.** Both suites re-run green to prove parity.
- **backtesting (vs quantstats/pyfolio/López de Prado): 228 → 248 tests, 89.96% coverage.**
  - `grid_search` now reports Deflated Sharpe (`n_trials`=grid size, variance-of-trials estimator) — the multiple-testing correction the sweep tooling needed. `PerformanceAnalytics` gains PSR + a trade-level block (avg win/loss, expectancy, payoff ratio, avg holding period, exposure time, MAE/MFE) via the existing FIFO round-trip engine, post-hoc (never in the hot loop). Folded into `generate_report()`.
- **Verification (REAL, main thread re-ran everything):** options **271** (99.38%) / portfolio **339** (95.59%) / backtesting **248** (89.96%) / market-data **326** (98.45%) / order-book **53/53 ctest + 89 py** (95.39%) — all green, all gates met. `ruff check` + `ruff format --check` clean across all 5 packages (22/37/29/32/13 files). Cross-package contract intact (backtester↔optimizer, shared `metrics` additive-only). Total Python now ~1273 + 53 C++.
- **User actions:** **NOTHING PUSHED** (per standing rules) — push `feature/quant-calculations` (6 commits past `main`) when ready, then it can merge to `main`. **Next-pass candidates (all optional, flagged by the gap analyses):** options Heston via characteristic function + barrier/digital exotics + SSVI arbitrage checks; portfolio walk-forward OOS evaluation harness + max-diversification objective + NCO; backtesting Monte-Carlo trade-resampling robustness + rolling/benchmark-relative stats (IR/TE) + borrow-fee model; market-data tick/volume/dollar bars (AFML) + realized-vol estimators; order-book execution-algo agents (TWAP/POV) + implementation-shortfall + queue-position/fill-probability.
